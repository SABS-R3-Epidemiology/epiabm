...
class voronoiPlotter():
    ...
    def point_in_region(self, point: np.ndarray,
        ...
        point : np.ndarray
            Point to consider
        grid_lim : typing.List[typing.List[float]]
            Spatial extent of plots, in the form
    ...
    def find_value_for_region(self, current_df: pd.DataFrame,
        ...
        assert name in current_df.columns.values, ("Column name not" +
                                                   "in dataframe")
        return row[name].values[0]  # Change to plot other characteristic

    def generate_colour_map(self, df: pd.DataFrame, name: str,
                            min_value: float = 0.0, cmap: plt.cm = cm.Reds):
    ...
    def find_time_points(self, time_series: pd.Series, num_times: int):
        ...
        idx = np.round(np.linspace(0, len(all_times) - 1, num_times))\
            .astype(int)
    ...
    def plot_time_grid(
        ...
        mapper = self.generate_colour_map(df, name=name)
        ...
        times = self.find_time_points(df["time"], plot_num)

        # Add each subplot
        for i, t in enumerate(times):
            self.plot_time_point(df, vor, name, t, grid_lim, axs[i], mapper)
    ...
    def generate_animation(
        ...
        name : str
            Name of quantity to extract
        grid_lim : typing.List[typing.List[float]]
            Spatial extent of plots, in the for
            [[min_x, max_x], [min_y, max_y]]
        save_path : str
        ...
        def animate(i):
            temp_fig, temp_ax = self.plot_time_point(df, vor, name, i,
                                                     grid_lim, ax,
                                                     mapper)
            if i == 0:
        ...
        if use_pillow:
            ...
            file_names = []
            for i, t in enumerate(times):
                t_fig, t_ax = self.plot_time_point(df, vor, name, t, grid_lim,
                                                   ax, mapper)
                if t == times[0]:
...
if __name__ == '__main__':
    ...
    anim = v.generate_animation(df, vor, name="InfectionStatus.InfectMild",

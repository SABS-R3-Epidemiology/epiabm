...
df = pd.read_csv(os.path.join(os.path.dirname(__file__),
                 "simulation_outputs/place_infections.csv"))
...
def plot_infections(data, infected_columns_mean: list,
    ...
    for k in range(len(infected_columns_mean)):
        # ax.errorbar(data['number_places'], data[infected_columns_mean[k]],
    ...
    plt.xticks(np.arange(min(data['number_places']), max(data['number_places'])+1, 1))
...
def plot_infections_mean_fit(data, infected_columns_mean: list,
    ...
    for k in range(len(infected_columns_mean)):
        ...
        slope, inter = np.polyfit(data['number_places'], data[infected_columns_mean[k]], 1)
        ax.errorbar(data['number_places'], slope*data['number_places'] + inter, fmt='.-', color=colors[k])
        ax.errorbar(data['number_places'], data[infected_columns_mean[k]],
                    fmt='o', capsize=5,
...
#                 ['midnightblue'], 'spatial_infectiousness_number_places_total')
# plot_infections(df, ['mean_house'], ['sd_house'], ['House'],
#                 ['slateblue'], 'spatial_infectiousness_number_places_house')
...
#                          ['Total', 'House', 'Place', 'Spatial'],
#                          ['midnightblue', 'slateblue', 'cyan', 'mediumblue'],
#                          'spatial_infectiousness_number_places')
...
class TestSimulation(TestMockedLogs):
    ...
    @patch('pyEpiabm.routine.simulation.tqdm', notqdm)
    @patch('pyEpiabm.routine.Simulation.write_to_serial_interval_file')
    @patch('os.makedirs')
    def test_run_sweeps_serial_interval(self, mock_mkdir, patch_write):
        mo = mock_open()
        self.inf_history_params["infectiousness_output"] = False
        self.inf_history_params["status_output"] = False
        self.inf_history_params["secondary_infections_output"] = False
    ...
    @patch('os.makedirs')
    def test_write_to_serial_interval_file(self, mock_mkdir, time=1):

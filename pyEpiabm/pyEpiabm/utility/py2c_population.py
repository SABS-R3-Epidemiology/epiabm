...
class _py2c_converter:
            ...
            for mc_i, m_cell in enumerate(cell.microcells):
                ...
                for person in m_cell.persons:
                    ...
                    assert person.microcell.cell == cell, \
                        "Person incorrectly linked to microcell outside of cell"
                    assert person.microcell == m_cell
                    assert not hasattr(person, "_index"), "Person already indexed (is person in two microcells?)"
                    person._index = (c_i, person.microcell._index[1], person_index)
                    person_index += 1
    ...
    def _validate_households(self):
        ...
        for household in self.households:
            for person in household.persons:
                if household._microcell_index is None:
                    ...
                    assert household._microcell_index == person.microcell._index, \
                        "Household cannot link two people in different microcells."
            if household._microcell_index is None:
        ...
        for household in self.households:
            ...
            self.py_population.cells[id[0]].microcells[id[1]]._n_households += 1
    ...
    def _add_people(self):
        ...
        for py_cell in self.py_population.cells:
            ...
            for py_mcell in py_cell.microcells:
                ...
                self.c_factory.add_persons(c_cell, c_mcell, len(py_mcell.persons))
    ...
    def _configure_people(self):
        ...
        for py_cell, c_cell in zip(self.py_population.cells, self.c_population.cells()):
            ...
            for py_person, c_person in zip(py_cell.persons, c_cell.persons()):
                ...
                params.susceptibility = 1.0 # Is this correct? each person doesn't have their own susceptibility?
    ...
    def _configure_households(self):
        ...
        for _, py_household in enumerate(self.households):
            ...
            for py_person in py_household.persons:
                ...
                assert c_person.set_household(hh_i), "Person was already in a different household"

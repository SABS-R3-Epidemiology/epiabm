#ifndef EPIABM_UTILITIES_THREAD_POOL_HPP
#define EPIABM_UTILITIES_THREOD_POOL_HPP
...
public:
    ...
    {
        return tasks_total - static_cast<ui32>(get_tasks_queued());
    }
    ...
    void parallelize_loop(const T1 &first_index, const T2 &index_after_last, const F &loop, ui32 num_blocks = 0)
    {
        ...
        T the_first_index = static_cast<T>(first_index);
        T last_index = static_cast<T>(index_after_last);
        ...
        ui64 total_size = static_cast<ui64>(last_index - the_first_index + 1);
        ui64 block_size = static_cast<ui64>(total_size / num_blocks);
        if (block_size == 0)
        {
            block_size = 1;
...
#endif // EPIABM_UTILITIES_THREAD_POOL_HPP
